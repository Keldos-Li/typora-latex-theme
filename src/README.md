...
**以上步骤执行完毕后， Typora 显示的结果可能并不符合您的预期，主要是一些字体与您的预期不符。这时，您需要安装本主题依赖的所有字体。**对于 macOS 和 Windows 用户，您可以在  [Keldos-Li/typora-latex-theme-fonts](https://github.com/Keldos-Li/typora-latex-theme-fonts) 仓库下载并安装您需要的字体； Linux 用户应确保相关字体在您的发行版上被正确安装。具体而言：  
...
- Linux 用户需安装 `Noto CJK` 家族的字体；以 Ubuntu 为例，用户可以执行 `sudo apt install fonts-noto-cjk` 进行安装。  
